...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fetchTracks = async () => {
      try {
        const response = await fetch("http://localhost:3000/user/getAllTracks");
        if (!response.ok) throw new Error("Failed to fetch tracks");

        const data = await response.json();
        console.log(data)
        // Ensure API response maps to expected Track structure
        const formattedTracks: Track[] = data.map((track: any) => ({
          title: track.title,  // Map 'title' from MongoDB to 'name'
          album: track.album,
          artist: track.artists,
          genre: track.genre,
          fileUrl: track.fileUrl, // Map 'previewUrl' from MongoDB to 'fileUrl'
          img: track.img,  // Map 'thumbnail' from MongoDB to 'cover'
        }));

        setTracks(formattedTracks);
        console.log(formattedTracks, "Fetched MongoDB tracks");
      } catch (error) {
  ...
  <div className="flex items-center justify-between mb-6">
    ...
    <div className="flex items-center gap-3">
     <img src={profileImg} alt="Profile" className="w-10 h-10 rounded-full" />
      ...
      <span className="text-white font-medium">Joel Francis</span>
    </div>
  ...
  <tbody>
    {tracks.map((track, index) => (
      <tr key={index} className="group hover:bg-[#242424] transition">
        {/* Track Image and Play Button */}
        <td className="py-2 w-12 relative">
          <div className="relative flex items-center w-10 h-10">
            ...
            <button
              ...
              className="absolute inset-0 flex items-center justify-center bg-black/50  p-2 opacity-0 group-hover:opacity-100 transition-opacity"
            >
